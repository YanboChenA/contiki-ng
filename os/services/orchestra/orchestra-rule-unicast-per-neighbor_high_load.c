/* Based on the rule unicast per neighbor rpl ns*/
#include "contiki.h"
#include "orchestra.h"
...
{
  /* Allocate more resources between nodes to get better performance*/
...
// static void hc_unicast_init(uint16_t sf_handle) {
//   slotframe_handle = sf_handle;
...
//   // 为每个节点计算并分配时间槽
//   for(uint16_t i = 0; i < ORCHESTRA_UNICAST_PERIOD; i++) {
//     uint16_t timeslot = (get_node_timeslot(&linkaddr_node_addr) + i) % ORCHESTRA_UNICAST_PERIOD;
//     uint8_t link_options = LINK_OPTION_RX;
//     if(i == 0) { // 第一个时间槽同时支持发送和接收
...
// }
/*---------------------------------------------------------------------------*/
struct orchestra_rule unicast_per_neighbor_HL = {
  init,
  new_time_source,
  select_packet,
  NULL,
