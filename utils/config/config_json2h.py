...
#         # Config limits definitions
#         config_limits = config['config_limits']
#         for key, value in config_limits.items():
#             file.write(f'#define {key} {value}\n')
#         file.write('\n')
...
#         file.write('const sensor_config_t sensor_config = {\n')
#         file.write(f'  send_interval = {net_config["send_interval"]},\n')
#         file.write(f'  packet_size = {net_config["packet_size"]},\n')
#         file.write(f'  max_runtime = {net_config["max_runtime"]},\n')
        
#         # Global Load Variation
#         file.write('  load_variation_global = {\n')
#         for i, variation in enumerate(config.get('load_variation_global', [])):
#             if i >= config_limits['MAX_LOAD_VARIATIONS']: break
#             file.write('    {')
#             file.write(f'start_time = {variation["start_time"]}, ')
#             file.write(f'end_time = {variation["end_time"]}, ')
#             file.write(f'interval = {variation["interval"]}, ')
...
#         # Node Specific Config
#         file.write('  node_specific = {\n')
#         for i, node in enumerate(config.get('node_specific', [])):
#             if i >= config_limits['MAX_NODE_SPECIFIC']: break
#             file.write('    {\n')
#             file.write(f'      node_id = {node["node_id"]},\n')
            
#             # Load Variations
#             file.write('      load_variations = {\n')
#             for j, lv in enumerate(node.get('load_variation', [])):
#                 if j >= config_limits['MAX_LOAD_VARIATIONS']: break
#                 file.write('        {')
#                 file.write(f'start_time = {lv["start_time"]}, ')
#                 file.write(f'end_time = {lv["end_time"]}, ')
#                 file.write(f'interval = {lv["interval"]}, ')
...
#             # Failures
#             file.write('      failures = {\n')
#             for k, failure in enumerate(node.get('failures', [])):
#                 if k >= config_limits['MAX_NODE_FAILURES']: break
#                 file.write('        {')
#                 file.write(f'failure_time = {failure["failure_time"]}, ')
#                 file.write(f'recovery_time = {failure.get("recovery_time", 0)}')
#                 file.write('},\n')
